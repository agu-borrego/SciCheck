...
def get_feature_vector(graph, triple, relations, remove_triple=False, original_positive=None, emb_dict=None, centrality_indices=None, rels_to_study=None, transform_model=None):
    res = []
    s, r, t = triple
        ...
        for i in rng:
            ...
            for path in paths:
                if i == 1:
                    ...
                    matrix[rels_dict[path[0][1]]][rels_dict[path[1][1]]][rels_dict[path[2][1]]][rels_dict[path[3][1]]] += 1
                elif i == 5:
                    matrix[rels_dict[path[0][1]]][rels_dict[path[1][1]]][rels_dict[path[2][1]]][rels_dict[path[3][1]]][rels_dict[path[4][1]]] += 1
    ...
    if transform_model:
        if s in embs:
            ...
            emb1 = transform_model.encode(s.split(":")[-1].replace("_", " "))
            embs[s] = emb1

        if t in embs:
            emb2 = embs[t]
        else:
            emb2 = transform_model.encode(t.split(":")[-1].replace("_", " "))
            embs[t] = emb2

        res.append(util.dot_score(emb1, emb2).item())
        res.append(util.cos_sim(emb1, emb2).item())
    ...
    if settings.USE_PATHS:
        ...
        for i in rng:
            ...
            if i == 1:
                ...
                for x, y, z, k in product(rng_rels, rng_rels, rng_rels, rng_rels):
                    matrix[x][y][z][k] = f"path_{rels_dict[x]}->{rels_dict[y]}->{rels_dict[z]}->{rels_dict[k]}_c4"
            elif i == 5:
                for x, y, z, k, j in product(rng_rels, rng_rels, rng_rels, rng_rels, rng_rels):
                    matrix[x][y][z][k][j] = f"path_{rels_dict[x]}->{rels_dict[y]}->{rels_dict[z]}->{rels_dict[k]}->{rels_dict[j]}_c5"
            
            header.append(f"totalPaths_c{i}")
            header += matrix.flatten().tolist()

    if settings.TRANSFORM_MODEL:  # Embeddings are being used
        for entid in (1,2):
            for k in range(settings.TRANSFORM_DIMS):
                header.append(f"emb_ent{entid}_pos{k}")

        header.append(f"emb_dot_prod")
        header.append(f"emb_sim_cosine")
